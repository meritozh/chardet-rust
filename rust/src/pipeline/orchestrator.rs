...
pub fn run_pipeline(
    ...
    for enc in &valid_candidates {
        if enc.is_multibyte {
            let score = ctx.mb_scores.get(enc.name)
                .copied()
                .unwrap_or_else(|| {
                    structural::compute_structural_score(data, enc, &mut ctx)
                });
            if score > 0.0 {
    ...
    if !structural_scores.is_empty() {
        ...
        if best_score >= STRUCTURAL_CONFIDENCE_THRESHOLD {
            let results = score_structural_candidates(data, &structural_scores, &valid_candidates, &mut ctx);
...
fn gate_cjk_candidates<'a>(
    ...
    for enc in candidates {
        if enc.is_multibyte {
            ...
            let byte_coverage = structural::compute_multibyte_byte_coverage(
                data, enc, ctx, Some(non_ascii_count)
            );
...
fn score_structural_candidates(
    ...
    candidates_to_score.extend(
        valid_candidates.iter()
            .filter(|e| !e.is_multibyte)
            .copied()
    );
